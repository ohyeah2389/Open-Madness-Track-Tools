...
## LOD Setup
The Level-of-Detail system can be used to swap out a high detail mesh for a lower one as the camera moves away from the object. This is usually done to save performance while preserving visual fidelity where needed. To set up a Level-of-Detail system for a set of meshes you want to swap out, do the following:
- Create at least two objects representing two different detail levels of the same visual item. Make sure they all reside in the same collection (you should not have to make a new collection for each LOD group).
- In the same collection as the LOD objects, create a new empty and place it near the LOD objects (ideally located at their shared origins). Name it `SMS_LOD_something`, where `SMS_LOD_` is a fixed prefix and `something` can be any unique text identifying that visual item.
- With the empty named with the `SMS_LOD_` prefix, a new panel "Madness LOD Control" will be available in its Object Data Properties panel. It contains a UI providing setup of the LOD system, showing two object picker slots with corresponding switch distance spinners and a button for adding more slots for more levels of detail. Use this UI to pick the different LOD level objects and set their switch distances. 
- Though this is not required for functionality, it may be useful to parent the LOD objects to the LOD control empty for better organization.
