...
static bool loadOBJGroups(const char* path,
                   std::vector<PxVec3>&          verts,
                   ...
                   bool fixRotation)
{
    ...
    while(std::getline(in,line))
    {
        ...
        {
            char c; double x,y,z; ss>>c>>x>>y>>z;
            if(fixRotation) z = -z;
    ...
    if(!groups.empty() && groups.back().indices.empty()) groups.pop_back();

    size_t triTotal = 0; for(auto& g:groups) triTotal += g.indices.size()/3;
    std::cout<<"OBJ: "<<verts.size()<<" verts, "<<triTotal<<" tris in "
             <<groups.size()<<" groups\n";
...
// Ordered by length (longest first) to ensure proper prefix matching
static const std::vector<MaterialMapping> TERRAIN_MATERIALS = {
    // Longer names first to avoid false matches
...
static uint32_t getMaterialIndex(const std::string& objectName) {
    ...
    for (const auto& material : TERRAIN_MATERIALS) {
        if (upperName.find(material.name) == 0) {  // Check if name starts with material name
...
int main(int argc,char** argv)
{
    ...
    PxPhysics*         physics    = PxCreatePhysics(PX_PHYSICS_VERSION,
                                                   *foundation, scale);
    if(!physics) {
    ...
    params.meshPreprocessParams = PxMeshPreprocessingFlags(0); // keep original
    params.targetPlatform       = PxPlatform::ePC;
    params.suppressTriangleMeshRemapTable = false; // Preserve remap table
    ...
    {
        if(grp.indices.empty()){ ++grpId; continue; }

        // ----- compact vertices for this group -------------------------
        RawMesh raw;
        std::vector<PxU32> remap(globalVerts.size(), UINT32_MAX);

        // Option 1: Preserve all vertices (better for round-trip fidelity)
        ...
        std::cout << "  Input: " << desc.points.count << " vertices, " 
                  << desc.triangles.count << " triangles\n";
        ...
        // Write mesh size
        chunk.insert(chunk.end(), reinterpret_cast<const uint8_t*>(&meshSize),
                                 reinterpret_cast<const uint8_t*>(&meshSize)+4);
        
        // Write mesh data
        chunk.insert(chunk.end(), cooked.getData(), cooked.getData() + cooked.getSize());
        
        // Write material index
        chunk.insert(chunk.end(), reinterpret_cast<const uint8_t*>(&materialIndex),
                                 reinterpret_cast<const uint8_t*>(&materialIndex)+4);

        chunks.push_back(std::move(chunk));

        std::cout<<"  cooked group "<<grpId<<" ("<<grp.name<<") : "
                 <<raw.verts.size()<<" verts, "
                 <<desc.triangles.count<<" tris, material: "<<materialIndex<<"\n";
        ++grpId;
    }
    ...
    for(const auto& chunk : chunks) {
        if(!dump(outPath, chunk, /*append*/true)) return 1;
    }
